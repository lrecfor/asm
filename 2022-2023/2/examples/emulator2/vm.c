...
#define     VM_JZ       16
#define     VM_JMP      17
#define     VM_LAST_INS 17
// количество инструкций
#define VM_COUNT_INST   (VM_LAST_INS + 1)

static char *vm_mnem[] = {"nop", "in", "out", "mov", "add", "sub", "xor", "and", "or", "hlt", "brk", "push", "pop", "call", "ret", "cmp", "jz", "jmp"};
...
static vm_ins_result vm_jz(vm_struct* vm);
...
vm_handler vm_handlers[VM_COUNT_INST] = {vm_nop, vm_in, vm_out, vm_mov, vm_add, vm_sub, vm_xor, vm_and, vm_or, vm_hlt, vm_break, vm_push, vm_pop, vm_call, vm_ret, vm_cmp, vm_jz, vm_jmp};
...
//
// записывает значение по адресу в память
//
BOOL vm_put_memory(
    vm_struct* vm,
    unsigned int addr,
...
//
// записывает значение по адресу в регистре в память
//
...
static vm_ins_result vm_push (vm_struct *vm) {
    ...
    if (vm_get_operand(vm, &ip->op1, &value) == VM_RESULT_OK){
        if (vm_put_memory(vm, vm->regs[VM_REG_SP] - 4, 4, &value)) {
            vm->regs[VM_REG_SP] = vm->regs[VM_REG_SP] - 4;
        }
    }
...
static vm_ins_result vm_call(vm_struct* vm) {
    vmopvalue_t value, old_addr;

    vm_instruction* instruction = vm_get_current_instruction(vm);

    if (vm_get_operand(vm, &instruction->op1, &value) == VM_RESULT_OK) {
        old_addr = vm->ip;
        vm_put_memory(vm, vm->regs[VM_REG_SP] - 4, 4, &old_addr);
        vm->regs[VM_REG_SP] = vm->regs[VM_REG_SP] - 4;
        vm->ip = value - VM_INSTRUCTION_SIZE;
    }
...
static vm_ins_result vm_ret(vm_struct* vm) {
    ...
    vm_get_memory(vm, vm->regs[VM_REG_SP], 4, &addr);
    vm->regs[VM_REG_SP] = vm->regs[VM_REG_SP] + 4;
    vm->ip = addr;
